...
function constolMessagesWithDoneReaction(isDisappear) {
  ...
  messageItems.forEach(item => {
    ...
    for (let i = 0; i < molecules.length; i++) {
      ...
      const reactions = molecules[i].children;
      for (let j = 0; j < reactions.length; j++) {
        if (reactions[j].tagName !== "BUTTON") continue;
        if (reactions[j].children.length == 0 ||
            reactions[j].children[0].tagName !== "IMG") continue;
        const emojiName= reactions[j].children[0].getAttribute("data-stringify-emoji");
        let isTarget = false;
